    ...
    <div className="basket-container">
        ...
        basket.map((item, i) => (
          <div className="basket-item shadow" key={item.id}>
            <div className="basket-item-product-details">
              <img
                src={`https://www.themoviedb.org/t/p/w92${item.image}`}
                alt={item.title}
              />
              <Link
                to={`/movie/${item.title.split(" ").join("")}`}
                key={item.id}
                state={item.id}
            ...
            <div className="basket-item-cost">
              ...
              <div className="basket-adjust">
                <Button
                  ...
                  className="basket-adjust-button"
                >
                  -
                </Button>
      ...
      {basket.length > 0 ? (
        <div className="basket-total shadow">
          ...
          <p>
            Total{" "}
            <span className="total-cost">£{(basketQty * 2.99).toFixed(2)}</span>
          </p>
        </div>
      ) : (
        ""
